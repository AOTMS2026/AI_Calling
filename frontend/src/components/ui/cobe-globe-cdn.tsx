...
export function GlobeCdn({
    ...
    useEffect(() => {
        ...
        function init() {
            ...
            globe = createGlobe(canvas, {
                ...
                markerColor: [0.1, 0.5, 1], // Blue markers for Light mode
                glowColor: [0.8, 0.8, 0.8], // Soft gray glow against white background
                markerElevation: 0.05,
                markers: markers.map((m) => ({ location: m.location, size: 0.035, id: m.id })),
                arcs: arcs.map((a) => ({ from: a.from, to: a.to, id: a.id })),
                arcColor: [0.1, 0.5, 1],
                arcWidth: 1.5, arcHeight: 0.35, opacity: 0.8,
            })
            function animate() {
    ...
    const pyramidFaceStyle = (nth: number): React.CSSProperties => {
        ...
        const colors = ["#3b82f6", "#2563eb", "#1e40af", "#1d4ed8"] // Light theme blue pyramids
        return {
    ...
    return (
        <div className={`relative w-full max-w-[450px] aspect-square select-none flex justify-center items-center ${className}`}>
            <style>{`
            ...
            {markers.map((m) => (
                ...
                >
                    ...
                    <span style={{
                        ...
                        background: "#00f0ff", padding: "2px 6px", borderRadius: 3,
